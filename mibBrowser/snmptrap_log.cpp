...
char SnmpTrapLog::separator[32];

void SnmpTrapLog::init_options(options_type * options)
/*
      * Function:
...
int SnmpTrapLog::realloc_output_temp_bfr(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, u_char ** temp_buf, options_type * options)
/*
      * Function:
...
int SnmpTrapLog::realloc_handle_time_fmt(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, options_type * options, netsnmp_pdu *pdu)
/*
      * Function:
...
void SnmpTrapLog::convert_agent_addr(struct in_addr agent_addr, char *name, size_t size)
{
...
int SnmpTrapLog::realloc_handle_ip_fmt(u_char ** buf, size_t * buf_len, size_t * out_len,
                      int allow_realloc,
...
int SnmpTrapLog::realloc_handle_ent_fmt(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, options_type * options, netsnmp_pdu *pdu)
/*
      * Function:
...
int SnmpTrapLog::realloc_handle_trap_fmt(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, options_type * options, netsnmp_pdu *pdu)
/*
      * Function:
...
int SnmpTrapLog::realloc_handle_auth_fmt(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, options_type * options, netsnmp_pdu *pdu)
/*
      * Function:
...
int SnmpTrapLog::realloc_handle_wrap_fmt(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, netsnmp_pdu *pdu)
{
    size_t          i = 0;
...
int SnmpTrapLog::realloc_dispatch_format_cmd(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, options_type * options, netsnmp_pdu *pdu, netsnmp_transport *transport)
/*
      * Function:
      *         .
      *
...
int SnmpTrapLog::realloc_handle_backslash(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, char fmt_cmd)
...
//extern snmp::SnmpTrapServer* trapServer;

//int SnmpTrapLog::realloc_format_trap(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, const char *format_str, netsnmp_pdu *pdu, netsnmp_transport *transport)
///*
//      * Function:
...
//    std::shared_ptr<TrapBuilder> builder = std::make_shared<TrapBuilder>();

//    builder->createTrap();
//    builder->addTime(QString::asprintf("%.4d-%.2d-%.2d %.2d:%.2d:%.2d",
...
//                                       now_parsed->tm_min,
//                                       now_parsed->tm_sec));
//    builder->addHostAddress(pdu);
//    builder->addVersion(pdu->version);
//    builder->addCommunity(pdu);
//    builder->addTrapOid(pdu);
//    builder->addVariable(pdu);
//    builder->addTrapType(pdu);
//    builder->addTrapCommand(pdu);
//    builder->addBlockType();

//    if (trapServer)
//    {
//        Event trapEvent;//Текущий event
//        QList<Event> trapEventList;//Список event

//        switch(builder->build()->getTrapType())
//        {
//        case Trap::TrapType::COLD_START:
//            trapEvent = trapServer->trapTranslator->translateColdStartTrap(builder->build().get());
//            trapEventList.append(trapEvent);
//            break;
//        case Trap::TrapType::WARM_START:
//            trapEvent = trapServer->trapTranslator->translateWarmStartTrap(builder->build().get());
//            trapEventList.append(trapEvent);
//            break;
//        case Trap::TrapType::LINK_DOWN:
//        case Trap::TrapType::LINK_UP:
//            trapEvent = trapServer->trapTranslator->translateLinkUpDownTrap(builder->build().get());
//            trapEventList.append(trapEvent);
//            break;
//        case Trap::TrapType::AUTHENTICATIONFAILURE:
//            trapEvent = trapServer->trapTranslator->translateAuthenticationFailureTrap(builder->build().get());
//            trapEventList.append(trapEvent);
//            break;
//        case Trap::TrapType::EGPNEIGHBORLOSS:
//            trapEvent = trapServer->trapTranslator->translateEgpneighborlossTrap(builder->build().get());
//            trapEventList.append(trapEvent);
//            break;
//        case Trap::TrapType::ENTERPRISESPECIFIC:
//            trapServer->trapTranslator->definitionFunction(builder->build().get());//Определение функции
//            trapEventList = trapServer->trapTranslator->translateEnterpriseSpecificTrap(builder->build().get());
//            break;
//        }
//        ///Перебор всех Event событий
//        for(auto &currentEventTrap : trapEventList)
//        {
//            ///Если нужно скрыть трап
//            if (currentEventTrap.noUseTrap)
//            {
//                qDebug() << "SnmpTrapLog" << POSF << "" << builder->build().get()->getHostAddress() << builder->build().get()->getTrapOid() << "noUse";
//                ++currentEventTrap.countNoUseEvent;
//                continue;
//            }
//            ///Если не нашли нужного фильтра в csv файле
//            else if (currentEventTrap.EmptyTrap)
//            {
//                ++currentEventTrap.countEmptyTrap;
//                ///Парсим при помощи библиотеки С и результат в buf
//                int result = translateAdditionalInfoTrap(buf, buf_len, out_len, allow_realloc, format_str, pdu, transport);
//                ///Если нет ошибок
//                if(result)
//                {
//                    trapServer->trapTranslator->translateEmptyTrap(builder->build().get(), currentEventTrap, buf, out_len);
//                    ///Отправляем по сигналу сформированный Event
//                    trapServer->emitServerTraps(trapEventList);
//                }
//            }
//            ///Отправляем целый Event
//            else
//            {
//                trapServer->emitServerTraps(trapEventList);
//            }
//        }
//    }
//    qDebug() << "SnmpTrapLog" << POSF << "done";

//    return 1;
//}

int SnmpTrapLog::translateAdditionalInfoTrap(u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, const char *format_str, netsnmp_pdu *pdu, netsnmp_transport *transport)
{
